<html>
<body>
<h2>Hello World!</h2>
<ul>
    <li><a href="/cabecerashttp/cabecera-request">Headers http request</a></li>
    <li><a href="/cabecerashttp/products.xls">Export to xls</a></li>
    <li><a href="/cabecerashttp/products.html">List products</a></li>
    <li><a href="/cabecerashttp/products.json">List as JSON</a></li>
    <li><a href="/cabecerashttp/time">Actual Date</a></li>
    <li><a href="/cabecerashttp/redirect">Redirect</a></li>
    <li><a href="/cabecerashttp/dispatcher">Dispatcher</a></li>
    <li><a href="/cabecerashttp/login.jsp">Login</a></li>
    <li><a href="/cabecerashttp/search.jsp">Find a product</a></li>
</ul>
</body>
</html>
